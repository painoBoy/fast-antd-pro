...
class AllOrderDetail extends Component {
  ...
  handleBackOrder =()=>{
    ...
    if(orderId){
        dispatch({
          type: 'query/getOrderNo',
          callback:(res)=>{
            if(res){
             router.push(`deliverdetail?${orderId}&&${res.distributionOrderNo}`);
            }
          }
        })
    ...
    if(!orderDetail){
      ...
      const action = (
        <Fragment>
              ...
              <Button  style={{position:'absolute',right:40}} type="primary" onClick={this.handleBackOrder}>配货</Button>
        </Fragment>
      );
      ...
      const description = (
        <DescriptionList className={styles.headerList} size="small" col="2">
          ...
          <Description term="确认日期"></Description>
