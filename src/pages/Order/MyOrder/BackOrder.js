...
export default class BackOrder extends React.PureComponent{
  ...
  columns = [
    ...
    },
    {
      ...
      render: (text, record) => (
        <div>
          {text==0 ?<Link to={`allorderdetail?${record.id}`}>配货</Link>:<a onClick={() => this.handleUpdateModalVisible(true, record)}>补货</a>}
          <Divider type="vertical" />
          <Link to={`viewdetail?${record.id}`}>查看</Link>
        </div>
  ...
  handleExportExcel = () => {
    ...
    const _headers = [{ k: 'id', v: '订单号' }, { k: 'strReceiverId', v: '院方采购人' },
            { k: 'lx', v: '部门' }, { k: 'strreceivertel', v: '联系电话' },{k:'datereceive',v:'确认时间'}]
